...
use hubris_server::{
    AppState, bind_with_port_fallback, build_router,
};
...
async fn main() {
    ...
    let data_dir =
        std::env::var("HUBRIS_DATA_DIR").map_or_else(
            |_| {
    ...
    let host = std::env::var("HUBRIS_HOST")
        .unwrap_or_else(|_| {
            if is_dev {
                "127.0.0.1".to_string()
            } else {
    ...
    let listener = if is_dev {
        ...
        let dev_port =
            base_port + DEV_BACKEND_PORT_OFFSET;
    ...
    {
        let state_file = PathBuf::from(&dev_tmp)
            .join(format!(
                "dev-{dev_id}.backend.json"
            ));
