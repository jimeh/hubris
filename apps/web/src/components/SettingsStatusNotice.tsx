...
import type { SettingsStatus } from "@/lib/theme/types";

export default function SettingsStatusNotice({
  status,
  variant = "app",
  className,
}: {
  ...
  const message = status.message ?? "The settings file could not be parsed.";
  ...
  return (
    <section
      aria-live="polite"
      className={cn(
        "relative overflow-hidden border border-amber-500/40 bg-[linear-gradient(135deg,rgba(245,158,11,0.16),rgba(251,191,36,0.08)_46%,rgba(120,53,15,0.14))] text-amber-950 shadow-[inset_0_1px_0_rgba(255,255,255,0.28)] dark:text-amber-50",
        isDialog ? "rounded-xl" : "border-x-0 border-t-0",
        className,
      )}
    >
      ...
      <div
        className={cn(
          "relative flex gap-3",
          isDialog ? "px-4 py-3.5 sm:px-5" : "px-4 py-3",
        )}
      >
        ...
        <div className="min-w-0 space-y-1">
          ...
          <p className="max-w-3xl text-sm leading-5 text-amber-950/88 dark:text-amber-50/88">
            Hubris is using the last known or default settings. Settings saves
            are blocked until the file becomes valid again.
          </p>
