...
function packagePathCandidates(relativePath: string): string[] {
  return [
...
function resolveExistingPath(candidates: string[], kind: string): string {
  ...
  throw new Error(`${kind} not found. Checked: ${candidates.join(", ")}`);
}
...
function defaultMonacoContributionManifestCandidates(): string[] {
  return [
    ...
    ...packagePathCandidates("monaco-editor/esm/vs/editor/editor.main.js"),
  ];
}
...
function languageRoot(): string {
  return resolveExistingPath(
    packagePathCandidates("monaco-editor/esm/vs/language/"),
    "Monaco language root",
  );
}
...
function generatedRustFile(): string {
  return fileURLToPath(
    new URL(
      "../../server/src/api/monaco_languages.generated.rs",
      import.meta.url,
    ),
  );
}
