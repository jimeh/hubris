<script lang="ts">
  ...
  $effect(() => {
    ...
    if (count > prevTabCount && tabListEl) {
      // Use tick-like delay so DOM has updated
...
<div class="flex h-full min-w-0 flex-col">
  <div
    class="relative flex min-h-9 items-center border-b border-tab-border bg-tab-bar px-1"
  >
    {#if canScrollLeft}
      <button
    ...
    {#if canScrollRight}
      <button
        class="absolute top-0 right-9 bottom-0 z-10 flex w-6 cursor-pointer items-center justify-center text-muted-foreground hover:text-foreground"
