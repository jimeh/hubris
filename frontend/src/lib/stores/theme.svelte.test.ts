...
describe("Theme store", () => {
  ...
  it("coerces legacy theme ids to hubris defaults and persists them", async () => {
    ...
    expect(store.activeTheme?.id).toBe("hubris-light");
  ...
  it("falls back to built-ins only and normalizes cached unknown ids", async () => {
    ...
    expect(store.allThemes.map((theme) => theme.id)).toEqual([
      "hubris-light",
      "hubris-dark",
    ]);
    expect(store.activeTheme?.id).toBe("hubris-dark");
