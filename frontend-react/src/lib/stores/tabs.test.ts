...
describe("Tab store", () => {
  ...
  it("loads tabs from snapshot sorted by position", async () => {
    ...
    expect(store.getState().tabs.map((t) => t.id)).toEqual(["b", "a"]);
  });
  ...
  it("reorder() resequences locally and calls API", async () => {
    ...
    await store.getState().reorder("w1", ["c", "a", "b"]);

    expect(mockReorderTabs).toHaveBeenCalledWith("w1", ["c", "a", "b"]);
    const worktreeTabs = store.getState().tabsForWorktree("w1");
